...
|------|------|
| `help` | 打印本帮助 |
| `alert [key on\|off]...` | 设置 SignalGetAlertMessage 返回值，可多组 key（见下表） |
...
### 3.2 Pull 槽：alert（SignalGetAlertMessage）

- **含义**：你设置的值会在设备管理端轮询时通过 `SignalGetAlertMessage()` 被读取，用于告警与喇叭控制逻辑（如 3m/7m 喇叭、双路轮播等）。
...
[pull] SignalGetAlertMessage: Enable3Alert=1 Enable7Alert=0 Alert3M=0 Alert7M=1
```

```text
alert enable3 on enable7 on 3m on 7m on
[pull] SignalGetAlertMessage: Enable3Alert=1 Enable7Alert=1 Alert3M=1 Alert7M=1
```

当 `Enable3Alert && Alert3M` 与 `Enable7Alert && Alert7M` 同时为真时，设备管理端会进入 3m/7m 喇叭轮播（此时为减少刷屏，喇叭控制输出为静默）。

### 3.3 Pull 槽：power（SignalGetBatteryButtonSignals）

- **含义**：你设置的值会在设备管理端轮询时通过 `SignalGetBatteryButtonSignals()` 被读取，用于电源按钮/继电器控制（0=None, 1=PowerOn, 2=PowerOff）。
...
```text
power on
[pull] SignalGetBatteryButtonSignals: on (1)
```

```text
power off
[pull] SignalGetBatteryButtonSignals: off (2)
```

```text
power none
[pull] SignalGetBatteryButtonSignals: none (0)
```

### 3.4 Push 槽：status（SignalSendDeviceStatus 缓存）

- **含义**：设备管理端定时聚合各驱动状态后，通过 `SignalSendDeviceStatus` 推送到客户端；程序只做缓存，**不**在每次推送时打印。输入 `status` 时从缓存读取并打印最近一次。
- **用法**：`status`
...
- 尚未收到过推送时：
  ```text
  [push] 尚未收到 SignalSendDeviceStatus，请稍后再试
  ```
- 已收到过推送时（示例）：
  ```text
  [push] DeviceStatus:
...
### 3.5 Push 槽：crane（SignalSendCraneState 缓存）

- **含义**：设备管理端通过 `SignalSendCraneState` 推送吊钩/小车距离等；程序只缓存，输入 `crane` 时打印最近一次。
- **用法**：`crane`
...
- 尚未收到过推送时：
  ```text
  [push] 尚未收到 SignalSendCraneState，请稍后再试
  ```
- 已收到过推送时（示例）：
  ```text
...
| Slot 类型 | 槽名称 | 测试方式 | 说明 |
|-----------|--------|----------|------|
| Pull | SignalGetAlertMessage | `alert [key on\|off]...` | 设置后由设备管理端轮询读取，影响告警/喇叭逻辑 |
| Pull | SignalGetBatteryButtonSignals | `power none\|on\|off` | 设置后由设备管理端轮询读取，影响电源按钮/继电器 |
| Push | SignalSendDeviceStatus | `status` | 按需打印最近一次推送的 DeviceStatus |
| Push | SignalSendCraneState | `crane` | 按需打印最近一次推送的 CraneState |
